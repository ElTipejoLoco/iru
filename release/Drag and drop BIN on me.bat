@echo off
set filename=Iru-v1.0
set file_type=BIN
set patch_file=iru_v1.0_patch.xdelta3
set multitrack_patch_file=iru_v1.0_multitrack_patch.xdelta3
...
if errorlevel 1 goto :MULTITRACKDRIFTING
goto :FIN

:MULTITRACKDRIFTING
echo.
echo Attempting multitrack patch instead...
patch_data\xdelta.exe -d -f -s "%~1" patch_data\%multitrack_patch_file% %filename%.bin
